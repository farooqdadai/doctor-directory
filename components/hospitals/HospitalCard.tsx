  ...
  return (
    <Link href={`/hospital/${hospital.slug}`} className="block h-full">
      <motion.div
        ...
        className="group bg-white rounded-2xl border border-gray-100 hover:border-brand-200 shadow-sm hover:shadow-xl hover:shadow-brand-700/10 transition-all duration-300 overflow-hidden h-full"
      >
        {/* Header with gradient */}
        <div className="h-28 bg-gradient-to-br from-brand-900 via-brand-700 to-brand-500 relative overflow-hidden">
          {/* Pattern overlay */}
          ...
          <div className="absolute -bottom-8 left-6">
            <div className="w-16 h-16 rounded-2xl bg-white shadow-xl shadow-brand-700/20 flex items-center justify-center border border-gray-100 group-hover:shadow-brand-700/30 group-hover:scale-105 transition-all duration-300">
              {hospital.logo ? (
          ...
          {hospital.googleRating && (
            <div className="absolute top-4 right-4 flex items-center gap-1.5 px-3 py-1.5 bg-white/95 backdrop-blur-sm rounded-xl text-sm font-bold shadow-lg">
        ...
        <div className="pt-12 px-6 pb-6">
          {/* Name */}
          <h3 className="text-lg font-bold text-gray-900 group-hover:text-brand-700 transition-colors duration-300 line-clamp-2 mb-3">
            {hospital.name}
          </h3>

          {/* Type Badge */}
          {hospital.type && (
            <span className="inline-flex items-center px-3 py-1.5 rounded-xl bg-brand-50/80 text-brand-800 text-xs font-semibold mb-3">
              {hospital.type}
            </span>
          )}
          ...
          <div className="flex flex-wrap gap-2 mb-5">
            ...
            {hospital.trauma && (
              <span className="inline-flex items-center px-3 py-1.5 rounded-xl bg-red-50 text-red-700 text-xs font-semibold">
            ...
            <div className="flex items-center gap-2">
              {hospital.telephone && (
                <a
                  ...
                  className="w-9 h-9 rounded-xl bg-gray-50 hover:bg-brand-50 flex items-center justify-center transition-colors duration-300"
                >
                  <svg className="w-4 h-4 text-gray-400 hover:text-brand-700 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
